			...
			case 2:
	            ...
	            case 1:
	            	searchjerrcy(p1,5);
	                  	break;
	            case 2:
	            	index = searchname(p1,5);
	            	printf("Index value is %d",index);
	            	if(index>=0)
	            	{			
						printf("\njerrcy no.%d\n Name %s\n Matchs %d\n Runs%d Wickets%d\n\n",p1[index].jerrcynum,p1[index].name,p1[index].matches,p1[index].runs,p1[index].wickets);
...
int searchname(player* p1, int n)
{
	int ch, i, result;
	char name[20];
	 printf("Enter the player name ");
	 fflush(stdin);   
	 // string che entry last-> null then we enter the int value -> so we free the space using str can free using flush.
	 //clr o/p form buffer and move the buffer date to console.
		 scanf("%s",&name);
		 for (i=0; i<n; i++)
		 {
